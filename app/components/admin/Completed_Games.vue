<template>
    ...
    <v-col cols="12" lg="10" class="d-flex justify-space-between align-center">
      <div class="d-flex align-center justify-center justify-sm-start ga-2 ga-lg-5 mt-2 mt-lg-5">
        ...
        <p class="shadowed-text text-subtitle-2 text-sm-subtitle-1 text-lg-h5 text-xl-h4 default-title-letter">
          Bitirdiğim Oyunlar
          <strong class="font-weight-bold default-title-letter">({{ completedGames?.length }})</strong>
        </p>
      </div>

      <v-btn icon="mdi-refresh" class="rounded text-caption text-lg-subtitle-2" :ripple="false" variant="text"
        rounded="xl" :color="isGettingCompletedGames ? 'green-accent-2' : 'grey-lighten-1'" @click="getCompletedGames"
        :size="smallScreen ? 'x-small' : 'small'" :loading="isGettingCompletedGames" />
    </v-col>

    <v-col cols="12" lg="10">
      <Admin_Game_Table :loading="isGettingCompletedGames" :arr="completedGames" :onDeleteClick="handleDeleteGame"
  ...
  <v-dialog v-model="isOpenConfirmationDialog" :max-width="600" style="
    ...
    <div class="delete-game-pop-up d-flex flex-column align-start ga-2 ga-lg-4 rounded pa-2 pa-lg-5">
      ...
      <div class="d-flex flex-column align-start ga-1">
        ...
        <p v-html="truncateText(activeGame?.description, 150)" class="text-caption text-grey-lighten-3"></p>
      </div>
      ...
      <div class="w-100 d-flex align-center justify-end ga-1 mt-2">
        <v-btn @click="isOpenConfirmationDialog = false" :ripple="false" class="rounded"
          :size="isExtraLargeScreen ? 'default' : 'small'" color="grey-lighten-2" variant="text"
        ...
        <v-btn @click="deleteThisGameFromDb(activeGame?.firestoreId)" :loading="isDeletingGameFromDb" :ripple="false"
          class="rounded" color="error" :size="isExtraLargeScreen ? 'default' : 'small'" variant="tonal"
  ...
  <!-- Game Detail Pop Up -->
  <v-dialog v-model="isOpenGameDetail" :max-width="600" style="
      background-color: rgba(0, 0, 0, 0.85);
      backdrop-filter: blur(0.7rem);
      -webkit-backdrop-filter: blur(0.7rem);
    ">
    <div class="game-detail-pop-up d-flex flex-column align-start ga-2 ga-lg-4 rounded pa-2 pa-lg-5">
      <v-btn @click="isOpenGameDetail = false" icon="mdi-close" color="grey-darken-1" class="close-btn ma-1"
      ...
      <div class="d-flex align-center ga-2">
        ...
        <div class="d-flex flex-column align-start">
          ...
          <p class="text-caption text-xl-subtitle-2 text-grey-darken-1 default-title-letter">
            {{ new Date(activeGame?.released).getFullYear() }}
          </p>
        </div>
      </div>
      ...
      <div class="d-flex flex-wrap align-center ga-2">
        <v-chip size="small" :ripple="false" class="rounded default-title-letter" color="grey-darken-1"
          text="Metacritic" />
      ...
      <!-- genres -->
      <div class="d-flex flex-wrap align-center ga-2">
        <v-chip size="small" :ripple="false" class="rounded default-title-letter" color="grey-darken-1" text="Genre" />

        <v-chip v-for="(genre, index) in activeGame?.genres" :key="index" :size="smallScreen ? 'x-small' : 'small'"
          color="green-accent-2" class="rounded-xl" variant="tonal" :ripple="false" :text="genre.name" />
      </div>
      ...
      <div class="d-flex flex-wrap align-center ga-2">
        <v-chip size="small" :ripple="false" class="rounded default-title-letter" color="grey-darken-1"
          text="Platform" />
        <template v-for="icon in getUniquePlatformIcons(activeGame?.platforms)" :key="icon">
      ...
      <!-- Stores -->
      <div class="d-flex flex-wrap align-center ga-2">
        <v-chip size="small" :ripple="false" class="rounded default-title-letter" color="grey-darken-1" text="Store" />
        <template v-for="s in activeGame?.stores ?? []" :key="s.id">
          <v-chip :size="smallScreen ? 'x-small' : 'small'" :ripple="false" class="rounded default-title-letter"
            color="primary" :text="s.store.name" />
        </template>
      </div>

      <v-divider v-if="activeGame?.description?.length > 250" color="white" class="w-100" />

      <!-- description -->
      <p class="text-caption text-lg-subtitle-2 text-grey-lighten-1 default-title-letter" v-html="displayedDescription">
      </p>

      <!-- toggle button -->
      <div v-if="activeGame?.description?.length > 250" class="d-flex align-center justify-end w-100">
        <v-btn variant="text" size="small" :ripple="false" class="text-grey-lighten-1"
  ...
  <!-- Add Game -->
  <v-dialog v-model="isAddGame" :max-width="600" style="
      background-color: rgba(0, 0, 0, 0.85);
      backdrop-filter: blur(0.7rem);
      -webkit-backdrop-filter: blur(0.7rem);
    ">
    <div class="add-game-pop-up d-flex flex-column align-center ga-2 ga-lg-4 rounded pa-2 pa-lg-5">
      <v-btn @click="isAddGame = false" icon="mdi-close" color="grey-darken-1" class="close-btn ma-1" variant="text"
        size="small" :ripple="false" />

      <div class="d-flex justify-center align-center ga-2">
        <p class="text-center text-subtitle-2 text-lg-subtitle-1 text-xl-h5 text-grey-darken-1 default-title-letter">
          Oyun Ekle
        </p>
      ...
      <v-text-field v-model="searchGameText" @input="searchGame" prepend-inner-icon="mdi-magnify" variant="outlined"
        ...
        placeholder="Black Myth Wukong..." :density="isExtraLargeScreen ? 'comfortable' : 'compact'" clearable />
      ...
      <transition name="slide-up">
        <v-row v-if="selectedGamesAfterResearch?.length > 0" class="w-100 mx-auto d-flex align-center" dense>
          <v-col cols="12" sm="6">
            <v-btn @click="addGameToDb" :loading="isAddingToDb"
              :text="`Tamamlanan Oyunlar Ekle (${selectedGamesAfterResearch?.length})`" size="small" :ripple="false"
              prepend-icon="mdi-plus" class="text-capitalize" block />
          </v-col>

          <v-col cols="12" sm="6">
            <v-btn @click="selectedGamesAfterResearch = []" text="Tüm Seçimleri Kaldır" size="small" :ripple="false"
      ...
      <transition name="slide-up">
        <v-row class="w-100" v-if="isAddedToDb">
          <v-col cols="12">
            <v-alert class="w-100 text-caption text-lg-subtitle-2" density="compact" color="success" variant="text"
  ...
  <!-- Toast -->
  <v-dialog v-model="notificationModels.isAvailable" :max-width="600" style="
      background-color: rgba(0, 0, 0, 0.85);
      backdrop-filter: blur(0.1rem);
      -webkit-backdrop-filter: blur(0.1rem);
    ">
    <div class="successfully-done-container d-flex flex-column align-center justify-center pa-5 rounded-xl">
      <v-btn @click="notificationModels.isAvailable = false"
