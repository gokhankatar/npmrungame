<template>
  <v-row :dense="isSmallScreen">
    ...
    <v-col v-for="(item, index) of game_genres" :key="item.slug" cols="6" lg="3">
      <v-card class="game-genre-card rounded-lg cursor-pointer" :ripple="false" @click="handleRouteGameGenre(item)">
        <v-img :src="item.background_img" class="game-genre-card-img" cover />
        <div class="w-100 game-genre-title pa-1 pa-lg-2 d-flex justify-center justify-sm-start align-center">
          <p class="text-white text-caption text-sm-subtitle-2 text-lg-subtitle-1 text-xl-h5 default-title-letter">
            {{ item.name }}
          </p>
        </div>
      </v-card>
